...
function startOtpTimer() {
  ...
  otpCountdown = setInterval(() => {
    ...
    if (secs <= 0) {
      ...
      sendOtpText.textContent = currentLang === 'ta' ? 'மீண்டும் அனுப்பு' : 'Resend';
    }
  }, 1000);
}
...
sendOtpBtn.addEventListener('click', async () => {
  ...
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    setErr('email', MSGS.email[currentLang]);
    return;
  }
  ...
  try {
    ...
    if (data.status === 'ok') {
      ...
      setErr('email', data.message || (currentLang === 'ta' ? 'OTP அனுப்ப முடியவில்லை' : 'Failed to send OTP'));
      sendOtpBtn.disabled = false;
    }
  } catch {
    setErr('email', currentLang === 'ta' ? 'நெட்வொர்க் பிழை. மீண்டும் முயற்சிக்கவும்.' : 'Network error. Try again.');
    sendOtpBtn.disabled = false;
  } finally {
...
document.getElementById('registration-form').addEventListener('submit', async (e) => {
  ...
  if (!emailVerified) {
    setErr('email', currentLang === 'ta'
      ? 'தொடர்வதற்கு முன் மின்னஞ்சலை சரிபார்க்கவும்'
